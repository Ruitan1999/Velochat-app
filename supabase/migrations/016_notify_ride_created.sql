-- Notify all participants of a ride chat that the ride was created (room title in notification).
-- Called by the app after creating the ride, chat room, and participants.
-- Only the room creator can invoke this RPC.
...
comment on function public.notify_ride_created(uuid) is
  'Sends push notification to all ride chat participants (except creator) that the ride was created. Call after inserting ride, chat_room, and chat_participants. Only room creator can call.';
