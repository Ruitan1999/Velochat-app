-- Track whether a participant currently has the chat room open.
-- The push trigger skips active participants (they see the message in real-time).

alter table public.chat_participants
  add column if not exists is_active boolean not null default false;

-- RPC: when p_active true, upserts participant and sets last_read_at = now(), is_active = true.
-- When p_active false, sets is_active = false. Runs as definer so RLS cannot block it.
...
-- Update only last_read_at (e.g. when user scrolls to bottom). Definer so RLS cannot block.
